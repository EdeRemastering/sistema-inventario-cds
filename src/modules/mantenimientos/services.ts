...
export function listMantenimientosProgramados(): Promise<MantenimientoProgramado[]> {
  return prisma.mantenimientos_programados.findMany({
    include: {
      elemento: {
        select: { id: true, serie: true, marca: true, modelo: true },
      },
    },
    orderBy: [{ fecha_mantenimiento: "desc" }, { creado_en: "desc" }],
...
export async function updateMantenimientosRealizadosByElemento(
  ...
  for (const { id } of ids) {
    await prisma.mantenimientos_realizados.update({ where: { id }, data: payload });
  }
...
export async function countMantenimientosPorEstado(): Promise<{
  ...
  const [pendientes, realizados, aplazados] = await Promise.all([
    prisma.mantenimientos_programados.count({
      where: { fecha_mantenimiento: { gte: start, lte: end }, estado: "PENDIENTE" },
    }),
    prisma.mantenimientos_programados.count({
      where: { fecha_mantenimiento: { gte: start, lte: end }, estado: "REALIZADO" },
    }),
    prisma.mantenimientos_programados.count({
      where: { fecha_mantenimiento: { gte: start, lte: end }, estado: "APLAZADO" },
    }),
  ]);

...
export const prestamoCreateSchema = z
  .object({
  ...
  .refine(
    (data) => {
      if (!data.fecha_estimada_devolucion) return true;
      return data.fecha_estimada_devolucion >= data.fecha_salida;
    },
    { message: "La fecha estimada de devolución debe ser mayor o igual a la fecha de salida", path: ["fecha_estimada_devolucion"] }
  );
